...
class SetTravelRouteScreen extends StatefulWidget {
  ...
  SetTravelRouteScreen({required this.scheduleId});
  @override
  _SetTravelRouteScreenState createState() => _SetTravelRouteScreenState();
}
...
void _saveRouteToDatabase() {
    if (_origin != null && _destination != null && _stops.isNotEmpty) {
      DatabaseReference routesRef = FirebaseDatabase.instance.reference().child('routesnew');
      ...
      Map<String, dynamic> routeData = {
        ...
        'stops': _stops.map((stop) => {'lat': stop.latitude, 'lng': stop.longitude}).toList(),
      };

      // Save routeId to the corresponding schedule in the 'schedules' node
    DatabaseReference schedulesRef = FirebaseDatabase.instance.reference().child('schedules');
